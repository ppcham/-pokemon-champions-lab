...
(() => {
  ...
  function pokemon(name, layer='icon'){
    ...
    return {kind:'pokemon',layer:l,key,id,path:`assets/pokemon/${l}/${id}.webp`,alt:key,fallback:false};
  }

  function item(name){
    const key=norm(name), slug=ITEM_MAP[key];
    return slug ? {kind:'item',key:slug,path:`assets/items/${slug}.webp`,alt:key,fallback:false} : fallback(key||'Poké Ball');
  }

  function type(name){
    const key=norm(name), slug=TYPE_MAP[key];
    return slug ? {kind:'type',key:slug,path:`assets/types/${slug}.webp`,alt:key,fallback:false} : fallback(key||'Poké Ball');
  }
  ...
  function audit(){
    return {
      ...
      fallback:FALLBACK,
      paths:{hero:'assets/pokemon/hero/{asset_id}.webp',icon:'assets/pokemon/icon/{asset_id}.webp',pixel:'assets/pokemon/pixel/{asset_id}.webp',item:'assets/items/{item-key}.webp',type:'assets/types/{type-key}.webp'}
    };
  }
